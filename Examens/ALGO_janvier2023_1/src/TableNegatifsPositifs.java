...
public class TableNegatifsPositifs {
    ...
    public boolean tousStrictementPlusGrands(int entier){
        ...
        if (nbPositifsEtNuls + nbNegatifs == 0)
            return true;
    ...
    public int supprimerMin(){
        ...
        // TODO
        int indiceMin = 0;   // initialisation du minimum avec le 1er élément (car table non vide)
        for (int i = 1; i < nbNegatifs+nbPositifsEtNuls; i++) {
            if (tableNP[i]<tableNP[indiceMin]){
                indiceMin = i;
            }
        }
        int min = tableNP[indiceMin];
