...
public class TriParSelection {
	...
	public static int[] tri5 (int[] t) {
		...
		for (int i = 0; i < t5.length; i++) {
			int indiceMin = i;
			for (int j = i+1; j < t.length; j++) {
				if (tCopie[j] < tCopie[indiceMin]) {
					indiceMin = j;
				}
			}
			int tmp = tCopie[i];
			tCopie[i] = tCopie[indiceMin];
			tCopie[indiceMin] = tmp;
			t5[i]=tCopie[i];
		}
		return t5;
	}
	...
	public static void triPartiel (int[] t, int indiceDebut, int indiceFin) {
		...
		// Ex supplementaire
		if(indiceDebut>indiceFin || indiceDebut<0 || indiceFin >= t.length)
			throw new IllegalArgumentException();

		for (int i = indiceDebut; i <= indiceFin; i++) {
			int indiceMin = i;
			for (int j = i+1; j <= indiceFin; j++) {
				if (t[j] < t[indiceMin]) {
					indiceMin = j;
				}
			}
