...
public class SudokuGeneral {
	...
	public boolean estPresentDansLigne(int nombre, int indiceLigne) {
		...
		for (int j = 0; j < grille[0].length; j++) {
			if (grille[indiceLigne][j] == nombre)
				return true;
		}
		return false;
	...
	public boolean estPresentDansRegion(int nombre, int indiceLigneSupGauche, int indiceColonneSupGauche) {
		...
		for (int i = indiceLigneSupGauche; i < tailleRegion + indiceLigneSupGauche; i++) {
			for (int j = indiceColonneSupGauche; j < tailleRegion + indiceColonneSupGauche; j++) {
	...
	public int nombreLePlusPresent() {
		...
		int[] repetitionsDuNombre = new int[grille.length + 1];

		int nombreLePlusPresent = 0;

		for (int i = 0; i < grille.length; i++) {
			for (int j = 0; j < grille.length; j++) {
				repetitionsDuNombre[grille[i][j]]++;
			}
		}

		for (int i = 1; i < repetitionsDuNombre.length; i++) {
			if (nombreLePlusPresent < repetitionsDuNombre[i])
				nombreLePlusPresent = i;
		}

		if (nombreLePlusPresent == 0)
			return -1;

		return nombreLePlusPresent;
